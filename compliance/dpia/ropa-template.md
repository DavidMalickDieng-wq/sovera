...
> Maintain two RoPAs: one as **controller** (Gardia operations) and one as
...
| 2 | Telemetry & observability (no PII payload) | App logs, metrics | Customer's end users (pseudonymous `sub` only) | Gardia SRE on-call (read-only) | Microsoft Azure (Log Analytics) | None | 730 days hot, archive 7 years | LAW with workspace-key encryption; access via PIM |
...
| 4 | Incident response | as (1) + Sentinel telemetry | as (1) | Gardia SOC, customer's DPO | Microsoft Sentinel | None | Per incident lifecycle | Sentinel workspace, MFA on access |
