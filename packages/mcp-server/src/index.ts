...
const GARDIA_URL = (process.env.GARDIA_URL ?? '').replace(/\/+$/, '');
const GARDIA_KEY = process.env.GARDIA_KEY ?? '';
const GARDIA_TENANT = process.env.GARDIA_TENANT?.trim() || undefined;
const READ_ONLY = /^(1|true|yes)$/i.test(process.env.GARDIA_READ_ONLY ?? '');

if (!GARDIA_URL || !GARDIA_KEY) {
  process.stderr.write(
    '[gardia-mcp] GARDIA_URL and GARDIA_KEY are required.\n' +
...
async function api<T = unknown>(path: string, init: FetchInit = {}): Promise<T> {
  const url = new URL(GARDIA_URL + (path.startsWith('/') ? path : '/' + path));
  ...
  const r = await fetch(url.toString(), {
    ...
    headers: {
      'authorization': `Bearer ${GARDIA_KEY}`,
      'x-api-key': GARDIA_KEY,
      'content-type': init.body ? 'application/json' : 'text/plain',
      'user-agent': 'gardia-mcp/0.1.0',
    },
...
const tools = [
  {
    name: 'gardia_sql',
    description:
      'Run a SQL query against the Gardia Postgres database. Read-only by default (SELECT only). ' +
      'Use this to inspect schema, fetch rows, count records, or join tables. ' +
      'If GARDIA_READ_ONLY=1 is set in env, DDL/DML statements are rejected.',
    inputSchema: {
      type: 'object',
      properties: {
        sql: { type: 'string', description: 'A single SQL statement to execute.' },
        tenant: { type: 'string', description: 'Optional tenant id (RLS scope). Falls back to GARDIA_TENANT env.' },
      },
      required: ['sql'],
    },
    ...
    handler: async (input: { sql: string; tenant?: string }) => {
      if (READ_ONLY && !/^\s*(select|with|explain|show)\b/i.test(input.sql)) {
        return err('GARDIA_READ_ONLY is enabled — only SELECT/WITH/EXPLAIN/SHOW are permitted.');
      }
      const body: Record<string, unknown> = { sql: input.sql };
      const t = input.tenant ?? GARDIA_TENANT;
      if (t) body.tenant = t;
  ...
  {
    name: 'gardia_tables_list',
    description: 'List all tables in the public/app schemas of the Gardia database, including row estimates and column counts.',
    inputSchema: { type: 'object', properties: {} },
    zod: z.object({}),
    handler: async () => ok(await api('/api/tables')),
  },
  {
    name: 'gardia_tenants_list',
    description: 'List all tenants in this Gardia project.',
    inputSchema: { type: 'object', properties: {} },
    zod: z.object({}),
    handler: async () => ok(await api('/api/tenants')),
  },
  {
    name: 'gardia_vector_search',
    description:
      'Semantic search over vector embeddings stored in Gardia. ' +
  ...
  {
    name: 'gardia_vector_embed',
    description:
      'Embed one or more text chunks into Gardia\'s vector store. ' +
      'Use this to index documentation, support tickets, or knowledge-base entries for later semantic retrieval. ' +
      'Disabled when GARDIA_READ_ONLY=1.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input: { texts: string[]; source: string; tenant?: string }) => {
      if (READ_ONLY) return err('GARDIA_READ_ONLY is enabled — write operations are disabled.');
  ...
  {
    name: 'gardia_realtime_publish',
    description: 'Publish a message to a Gardia realtime channel (Azure Web PubSub). Subscribers will receive it instantly. Disabled when GARDIA_READ_ONLY=1.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'gardia_blob_list',
    description: 'List blobs in a Gardia storage container.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'gardia_logs',
    description: 'Query recent application logs from the Gardia Functions backend (App Insights). Useful for debugging.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'gardia_compliance_status',
    description: 'Return the current compliance posture of this Gardia project (HDS, GDPR, EU data residency, encryption, RBAC).',
    inputSchema: { type: 'object', properties: {} },
    zod: z.object({}),
...
server.setRequestHandler(ListResourcesRequestSchema, async () => ({
  resources: [
    { uri: 'gardia://project', name: 'Project info', description: 'Gardia project URL, tenant scope, and read-only flag.', mimeType: 'application/json' },
    { uri: 'gardia://schema',  name: 'Database schema', description: 'Tables, columns, and row estimates.', mimeType: 'application/json' },
  ],
}));

server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
  const uri = req.params.uri;
  if (uri === 'gardia://project') {
    return { contents: [{ uri, mimeType: 'application/json', text: JSON.stringify({ url: GARDIA_URL, tenant: GARDIA_TENANT ?? null, readOnly: READ_ONLY }, null, 2) }] };
  }
  if (uri === 'gardia://schema') {
...
async function main() {
  ...
  process.stderr.write(`[gardia-mcp] connected · url=${GARDIA_URL} · tenant=${GARDIA_TENANT ?? '(none)'} · readOnly=${READ_ONLY}\n`);
}
main().catch(e => { process.stderr.write(`[gardia-mcp] fatal: ${(e as Error).message}\n`); process.exit(1); });
